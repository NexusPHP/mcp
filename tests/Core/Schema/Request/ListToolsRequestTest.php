...
final class ListToolsRequestTest extends TestCase
{
    ...
    public static function provideFromArrayRejectsInvalidWireDataCases(): iterable
    {
        yield 'missing id' => [
            ...
            'ListToolsRequest wire data missing "id".',
        ];

        yield 'id not int or string' => [
            ['jsonrpc' => '2.0', 'id' => [], 'method' => 'tools/list'],
            'ListToolsRequest wire "id" must be int or string, array given.',
        ];

        yield 'params not an object' => [
            ['jsonrpc' => '2.0', 'id' => 1, 'method' => 'tools/list', 'params' => 'bad'],
            'ListToolsRequest wire "params" must be an object, string given.',
        ];

        yield 'params list-keyed' => [
            ['jsonrpc' => '2.0', 'id' => 1, 'method' => 'tools/list', 'params' => ['x']],
            'ListToolsRequest wire "params" must be a string-keyed object.',
        ];
    }
}
