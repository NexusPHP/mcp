...
- **`Server::builder()` / `Client::builder()` are the canonical construction entry points and are deliberately kept.** Each is a static factory returning the matching builder. The product-to-builder reference is intra-namespace and violates no layering rule. The builder vocabulary (`setServerInfo` / `addTool` / `build`) is conventional naming and is not renamed for the sake of a distinct-looking surface. Differentiate on substance (strict typing, no reflection magic, mutation-tested correctness) and documented positioning. Dropping the static accessor or renaming the vocabulary trades a discoverable front door for cosmetic novelty.
...
- Each concrete request/notification overrides `static method(): string` to return the JSON-RPC method name as a literal. That accessor is the single source of truth: registry, parser, error messages, and tests all read it. Do not introduce a `JSONRPC_METHOD` constant. The method name is polymorphic per-subclass behaviour and belongs in a method, not a constant.
