...
| `stdio-client` | Stdio MCP client that spawns `stdio-server` as a subprocess and drives it through the typed `Client` API: `discover`, `listTools`, `callTool` (with streaming `onProgress`), `readResource`, `listPrompts`. | [stdio-client.php](stdio-client.php) |
