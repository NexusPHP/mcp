...
- **Unconditional `logging: {}` capability advertising is honest.** `ServerBuilder::deriveCapabilities()` emits `logging: []` even when no PSR-3 logger is registered. The MCP `logging` capability is two things, neither tied to that PSR-3 logger: handling `logging/setLevel` (the `SetLevelRequestHandler` is always registered) and emitting `notifications/message` to the client via `ServerContext::log()` (gated only by the level gate). The asymmetry with the deliberately-omitted `listChanged` is justified: log notifications can fire, list-changed notifications cannot. Do not flag the unconditional `logging` slot as dishonest advertising.

- **`StdioClientTransport` SIGKILLing the subprocess without `join()` is accepted.** On close it closes stdin and sends `\SIGKILL` without `join()`-ing to reap the exit status. amphp reaps the handle on GC, so there is no deterministic leak. An active-reap (`async(fn() => $process->join())->ignore()`) is deliberately not added: it is not unit-testable to the repo's 100% MSI bar (the `Process` is built internally, `->ignore()` swallows the result), it is stdio-specific, and the benefit is marginal. Revisit only if a real leak surfaces with a long-lived parent spawning many short-lived servers.
