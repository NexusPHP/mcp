...
`requestState` continuation token. A tool that asks for input through [`InputRequiredResult`](input-required.md)
...
- A `CallToolResult` completes the task, `isError` included. A tool-level failure is `completed` with
...
- A protocol exception fails the task and inlines `error.code` and `error.message`. `failed` is reserved for
  protocol errors.
- Cooperative cancellation settles the task as `cancelled`. `tasks/cancel` cancels the fiber's token.
...
The tasks methods carry the [SEP-2243 routing headers](../transports.md). `Mcp-Name` mirrors `params.taskId` on
