...
- **No redirects.** On the authorization legs, nothing read from an answer that arrived from a URL other than
  the one it was sent to is trusted, and every leg, metadata discovery included, runs on a client that
  follows no redirect. A `3xx` on a credentialed request comes back to the decorator, which refuses it
  unless the target is the canonical resource or under its path, so the credential never travels to the
  target. A
  redirected well-known probe reads as nothing served there, and the next candidate is tried. Since an
  origin carries its scheme, a downgrade from `https` to cleartext on the same host is refused like any
  other hop. A request the decorator sent no credential with is left alone, redirects and all.
- **Audience binding.** Server-side, a token whose audience does not name this server is refused. Client-side,
  a token is sent only to the resource it was obtained for, or a path under it. Another path on the same
  origin, another tenant included, gets the request without the credential.
