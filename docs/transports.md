...
| `RequestBodySizeLimitMiddleware` | `413` | Added only when you pass a cap. Measures the buffered body, so a streaming body whose size is unknown passes through to the host's own limit. Registering a tool store makes the size determinate, because the middleware above re-seats the body it read, so the cap then applies to a streaming body too. |
